...
// ── DOM refs ──────────────────────────────────────────────────────────────────
const tasksContainer = document.getElementById('ctTasksContainer');
...
function buildProgressSection(t) {
  ...
  // No baseline yet — backend couldn't find prior data for this category
  ...
  // null = no records at all; 0 or positive = records present (COALESCE from DB)
  ...
  const currentStr = periodTarget !== null
    ? `${displayCurrent.toFixed(1)} / ${periodTarget.toFixed(1)} kg CO₂e`
    : `${displayCurrent.toFixed(1)} kg CO₂e`;

  const remainingHtml = periodTarget !== null
    ? `<div style="font-size:11px;color:var(--color-text-muted);">Kalan: <strong>${Math.max(0, periodTarget - displayCurrent).toFixed(1)} kg CO₂e</strong></div>`
    : '';
  ...
  // CTA button — only show when there are no current-period records
  ...
  return `
    <div class="hh-task-progress">
      ...
      <div style="font-size:11px;color:var(--color-text-muted);display:flex;flex-direction:column;gap:3px;margin-bottom:6px;">
        <div>Aylık Baz: <strong>${baseline.toFixed(1)} kg CO₂e</strong></div>
        ${periodTarget !== null ? `<div>Dönem Hedefi: <strong>${periodTarget.toFixed(1)} kg CO₂e</strong></div>` : ''}
...
function taskCard(t) {
  ...
  const statusControl = selectOptions
    ? `<select class="hh-status-select" data-task-id="${t.id}" data-status="${t.status}">${selectOptions}</select>`
    : `<span class="status-badge ${getTaskStatusClass(t.status)}">${getTaskStatusLabel(t.status)}</span>`;
    ...
    <div class="hh-task-card${isDimmed ? ' hh-task-card--dim' : ''}" data-task-id="${t.id}">
      ...
      <div class="hh-task-card-footer">
        <div style="display:flex;flex-direction:column;gap:2px;">
          ...
          ${t.target_pct ? `<span style="font-size:11px;color:var(--color-text-muted);">Hedef: %${parseFloat(t.target_pct)} azaltım</span>` : ''}
        </div>
...
function renderTasks() {
      ...
      <div class="hh-empty">
        <div class="hh-empty-icon">
          <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"
               stroke-linecap="round" stroke-linejoin="round" style="color:var(--color-text-muted);opacity:0.7;">
  ...
  tasksContainer.querySelectorAll('.hh-status-select').forEach(sel => {
    sel.addEventListener('change', () =>
      handleStatusChange(sel.dataset.taskId, sel.value, sel.dataset.status, sel)
    );
  });
