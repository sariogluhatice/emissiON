...
const ROLE_LABELS = {
  individual: "Bireysel",
  household: "Hane Yöneticisi",
  company: "Kurum Yöneticisi",
  admin: "Yönetici",
};
...
function renderSection(section, answers) {
  ...
  if (populated.length === 0) {
    ...
    populated.forEach((key) =>
      list.appendChild(renderFieldRow(key, answers[key])),
    );
...
async function loadProfile() {
  try {
    ...
    const editEmailEl = document.getElementById("editEmailDisplay");
    if (editEmailEl) editEmailEl.value = u.email || "";
    const editNameEl = document.getElementById("editName");
    if (editNameEl) editNameEl.value = u.name || "";
    const roleEl = document.getElementById("profileRole");
    if (roleEl) roleEl.textContent = ROLE_LABELS[u.role] || u.role || "—";

    const since = new Date(u.created_at).toLocaleDateString("tr-TR", {
      year: "numeric",
      month: "long",
    });
    const sinceEl = document.getElementById("profileSince");
    if (sinceEl) sinceEl.textContent = `Üyelik başlangıcı: ${since}`;
    ...
    PROFILE_SECTIONS.filter(
      (s) => s.roles === "all" || s.roles.includes(u.role),
  ...
  ?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      document.getElementById("profileAvatar").textContent =
        updated.name?.charAt(0).toUpperCase() || "?";
...
document
  ...
  ?.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      setMsg(
        "changePasswordMsg",
        err.message || "Parola değiştirilemedi.",
        true,
      );
    } finally {
      const btn = document.getElementById("changePasswordBtn");
