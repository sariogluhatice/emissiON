...
if (welcomeEl)
  welcomeEl.textContent = user?.name ? user.name.split(" ")[0] : "Misafir";
...
(async () => {
  ...
  if (rateResult.status === "fulfilled" && rateResult.value.ok) {
    try {
      ...
      console.log(
        `[inflation] Live USD/TRY rate: ${tryRate}, multiplier: ${inflationFactor.toFixed(3)}`,
      );
    } catch {
      /* ignore parse error */
    }
    ...
    console.warn(
      "[inflation] Could not fetch live rate, falling back to baseline prices.",
    );
  }
  ...
  if (fuelResult.status === "fulfilled" && fuelResult.value.ok) {
    try {
      ...
      if (districts.length > 0) {
        ...
        if (fuelPrice && fuelPrice > 20) {
          ...
          console.log(
            `[fuel-api] Live fuel price: ${fuelPrice} TL, transport multiplier: ${liveFuelMultiplier.toFixed(2)}`,
          );
        }
      }
    } catch {
    ...
    console.warn(
      "[fuel-api] Could not fetch live fuel prices, falling back to baseline multipliers.",
    );
  }
  ...
  try {
    ...
    if (document.getElementById("statTotal")) {
      ...
      if (card) {
        card.setAttribute(
          "data-tooltip",
          `Tüm zamanlar: ${stats.total.toLocaleString("tr-TR")} kg CO₂e`,
        );
      }
    }
    if (document.getElementById("statEntries"))
      document.getElementById("statEntries").textContent = stats.entries;
    if (document.getElementById("statTopCat"))
    ...
    if (document.getElementById("statSavings")) {
      ...
      if (card) {
        card.setAttribute(
          "data-tooltip",
          `Tüm zamanlar: ${Math.round(lifetimeCost).toLocaleString("tr-TR")} TL`,
        );
      }
    }
    ...
    try {
      ...
      if (spotlightEl) {
        ...
        if (pred && typeof pred === "object") {
          pred =
            pred.text || pred.content || pred.value || pred.analysis || null;
        }
        if (typeof pred === "string" && pred.trim()) {
          spotlightEl.textContent = pred.trim();
        } else {
          spotlightEl.textContent =
            "Analiz hazırlanıyor. Lütfen biraz sonra tekrar bakın.";
        }
      }
    } catch {
...
const BADGE_DESC = {
  earth_friend:
    "Uygulamaya kayıt olarak sürdürülebilirlik yolculuğuna başladın.",
  ...
  data_expert:
    "20+ kayıtla verilerini titizlikle izleyen bir analiz uzmanısın.",
  ...
  carbon_aware:
    "300 XP biriktirerek yüksek karbon bilincine sahip olduğunu gösterdin.",
...
function _showGamSkeleton() {
  ...
  if (levelEl) {
    levelEl.textContent = "Yükleniyor…";
    levelEl.style.opacity = "0.35";
  }
  if (xpText) {
    xpText.textContent = "— XP";
    xpText.style.opacity = "0.35";
  }
...
function renderGamification(stats) {
  ...
  if (banner) {
    ...
    if (xpFill) {
      requestAnimationFrame(() => {
        xpFill.style.width = `${stats.progressPercent}%`;
      });
    }
    if (xpNext) {
      xpNext.textContent =
  ...
  if (badgesCard && badgesList && stats.badge_defs) {
    ...
    if (badgesCount)
      badgesCount.textContent = `${earned.length} / ${stats.badge_defs.length} kazanıldı`;
    ...
    [...earned, ...unearned].forEach((b) => {
      ...
      const desc = b.earned ? BADGE_DESC[b.id] || "" : "Henüz kazanılmadı";

      const uiIcon =
        BADGE_SVG_ICONS[b.id] ||
...
async function loadHouseholdTasks() {
  ...
  try {
    ...
    if (
      err.message?.includes("üye olmanız") ||
      err.message?.includes("Hane bulunamadı") ||
      err.message?.includes("yetkiniz yok")
    ) {
      card.style.display = "block";
      listEl.innerHTML = `
