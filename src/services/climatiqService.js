...
const FOOD_ACTIVITY_MAP = {
  beef_red_meat: "consumer_goods-type_meat_products_beef",
...
class ClimatiqService {
  ...
  async calculateFlightEmission(from, to, flightClass = "economy", flightType = null) {
    ...
    if (!start || !end) {
      throw new Error(
        `"${!start ? from : to}" için havalimanı bulunamadı. Lütfen IATA kodlarını (örn. IST) veya bilinen isimleri kullanın.`,
      );
    }
...
module.exports = Object.assign(new ClimatiqService(), { FOOD_ACTIVITY_MAP });
